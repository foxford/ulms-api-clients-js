...
const brokenClient = {
  ...
  enterRoom: jest.fn().mockImplementation(
    () =>
      new Promise((resolve, reject) => {
        reject(new Error('From Client'))
      })
  ),
}
...
describe('enterRoom should work', () => {
  it('enterRoom Throwing error', async () => {
    await expect(enterRoom(brokenClient, roomId, agentId, 100)).rejects.toThrow(
      'From Client'
    )
  ...
  it('enterRoom is ok', async () => {
    await expect(enterRoom(client, roomId, agentId, 100)).resolves.toBe(
      undefined
    )
